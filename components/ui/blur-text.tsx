  ...
  return (
    ...
    >
      {elements.map((segment, index) => {
        ...
        const shouldBeBold = boldWords.some(keyword => 
          segment.toLowerCase().includes(keyword.toLowerCase()) || 
          keyword.toLowerCase().includes(segment.toLowerCase())
        );

        return (
          <motion.span
            className={`inline-block will-change-[transform,filter,opacity] ${shouldBeBold ? 'font-semibold' : ''}`}
            key={index}
